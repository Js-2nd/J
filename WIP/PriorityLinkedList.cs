...
namespace J
{
	...
	{
		readonly SortedDictionary<int, HashedLinkedList<T>> main;
		...
		{
			main = new SortedDictionary<int, HashedLinkedList<T>>();
		...
		public void Add(T item, int priority = 0)
		{
			...
			main.GetOrAdd(priority, _ => new HashedLinkedList<T>(cmp)).Add(item);
		}
		...
		{
			var list = main.GetOrDefault(priority);
